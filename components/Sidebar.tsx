        ...
        <div className="flex flex-col h-full relative z-10 text-[10px]">
          <div className="p-3 border-b border-white/5">
            <div className="flex items-center gap-2 mb-4 group cursor-default">
              <div className="w-7 h-7 glass-panel rounded-lg flex items-center justify-center bg-gradient-to-tr from-nexus-accent/20 to-purple-500/20 border-white/10 shadow-lg text-nexus-accent scale-75"><BrainIcon /></div>
              <div className="flex flex-col leading-none">
                <span className="text-[11px] font-black italic tracking-tighter text-white uppercase">NEXUS</span>
            ...
            <div className="relative">
              <div className="absolute left-2.5 top-2 text-gray-700 scale-75"><SearchIcon /></div>
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-3 custom-scrollbar">
            <div onClick={() => onUpdateSettings({ ...appSettings, isAgentMode: !appSettings.isAgentMode })} className={`p-2.5 rounded-xl border transition-all cursor-pointer flex flex-col gap-1 ${appSettings.isAgentMode ? 'bg-nexus-purple/10 border-nexus-purple/40 shadow-glow text-nexus-purple' : 'bg-white/[0.02] border-white/5 text-gray-500 hover:bg-white/5'}`}>
            ...
            <div className="pt-2">
               ...
               <div className="space-y-0.5">
                {sessions.filter(s => s.title.toLowerCase().includes(searchQuery.toLowerCase())).map(session => (
                  <div key={session.id} onClick={() => onSelectSession(session.id)} className={`group relative flex items-center px-3 py-2 rounded-lg cursor-pointer transition-all border ${currentSessionId === session.id ? 'bg-nexus-accent/10 border-nexus-accent/20 text-white shadow-glow' : 'text-gray-500 hover:bg-white/5 border-transparent'}`}>
          ...
          <div className="p-3 bg-black/40 border-t border-white/5 space-y-2">
            ...
            <div className="flex items-center justify-between p-2 glass-panel rounded-xl border-white/10">
               <div className="flex items-center gap-2 min-w-0">
                 <div className="w-6 h-6 rounded-lg bg-nexus-accent/20 flex items-center justify-center border border-white/10 text-[9px] font-black text-nexus-accent uppercase overflow-hidden">
                    {user.avatar ? <img src={user.avatar} className="w-full h-full object-cover" /> : <div className="text-nexus-accent font-black uppercase text-[10px]">{user.name?.[0] || 'O'}</div>}
                 </div>
           ...
           <div className="glass-panel w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl flex flex-col relative z-[111] max-h-[85vh] border border-white/10 animate-in zoom-in-95 duration-300 text-[10px]">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar bg-nexus-950/40">
                 <section>
                    <div className="flex items-center gap-2 mb-4">
                       ...
                       <h3 className="font-black text-white uppercase tracking-widest flex items-center gap-2"><CommandIcon /> Access_Control</h3>
                    </div>
                    <div className="p-4 iphone-group bg-white/[0.02] border border-white/5 space-y-3">
                       <div className="flex items-center justify-between">
                          <div className="flex flex-col">
                            <span className="font-black text-gray-500 uppercase tracking-widest">Active_Token</span>
                            <span className="text-nexus-accent font-mono">{user.inviteCode}</span>
                          </div>
                       ...
                       {user.isOwner && (
                         <div className="pt-3 border-t border-white/5 space-y-3">
                            <div className="flex items-center justify-between">
                               <span className="text-gray-600 uppercase tracking-widest font-bold">Invite_Registry</span>
                               <button onClick={generateCode} className="text-nexus-accent font-black uppercase hover:underline">+ Gen</button>
                            </div>
                            <div className="grid grid-cols-2 gap-1.5 max-h-24 overflow-y-auto custom-scrollbar">
                               {activeCodes.map(code => (
                                 <div key={code} className="flex items-center justify-between p-1.5 bg-black/40 rounded-lg border border-white/5 group">
                                    ...
                                    <button onClick={() => revokeCode(code)} className="opacity-0 group-hover:opacity-100 text-red-500 hover:scale-110 transition-all">✕</button>
                                 </div>
                               ))}
                            </div>
                         </div>
                 ...
                 <section>
                    <div className="flex items-center gap-2 mb-4">
                       <div className="w-1 h-4 bg-nexus-emerald rounded-full"></div>
                       <h3 className="font-black text-white uppercase tracking-widest flex items-center gap-2"><AgentIcon /> Agent_Profile</h3>
                    </div>
                    <div className="p-4 iphone-group bg-white/[0.02] border border-white/5 space-y-3">
                       <div className="grid grid-cols-3 gap-1 bg-black/40 p-1 rounded-xl border border-white/5">
                         {(['general', 'researcher', 'developer'] as const).map((type) => (
                           <button 
                             ...
                             className={`py-2 rounded-lg text-[9px] font-black uppercase transition-all ${tempSettings.agentType === type ? 'bg-nexus-accent text-black shadow-glow' : 'text-gray-600 hover:text-gray-400'}`}
                           >
                             {type}
                           </button>
                         ))}
                       </div>
                       <p className="text-[8px] text-gray-700 font-mono italic uppercase px-1">Adjusts specialized system instructions for neural agent.</p>
                    </div>
                 </section>

                 <section>
                    <div className="flex items-center gap-2 mb-4">
                       <div className="w-1 h-4 bg-nexus-purple rounded-full"></div>
                       <h3 className="font-black text-white uppercase tracking-widest flex items-center gap-2"><ActivityIcon /> Engine_Params</h3>
                    </div>
                    <div className="p-4 iphone-group bg-white/[0.02] border border-white/5 space-y-5">
                       <div className="space-y-2">
                          <div className="flex justify-between font-mono uppercase tracking-widest text-[9px]">
                          ...
                          <input 
                            ...
                            className="w-full accent-nexus-accent opacity-50 hover:opacity-100" 
                          />
                       </div>

                       <div className="flex items-center justify-between pt-1">
                          <div className="flex flex-col">
                            <span className="font-black text-gray-400 uppercase tracking-widest">Grounding</span>
                            <span className="text-[8px] text-gray-700 font-mono">SEARCH_PROTO</span>
                          </div>
                          <button 
                            onClick={() => setTempSettings({...tempSettings, useSearch: !tempSettings.useSearch})}
                            className={`w-10 h-5 rounded-full relative transition-all duration-300 ${tempSettings.useSearch ? 'bg-nexus-accent shadow-glow' : 'bg-white/10'}`}
                          >
                             <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-transform duration-300 ${tempSettings.useSearch ? 'translate-x-6' : 'translate-x-1'}`}></div>
                          </button>
                       </div>
                    </div>
                 </section>
              ...
              <div className="p-6 border-t border-white/5 bg-black/20">
                 <button 
                   onClick={handleSave} 
                   disabled={isSaving} 
                   className={`w-full py-3.5 rounded-2xl font-black uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-2 ${isSaving ? 'bg-emerald-500 text-black shadow-glow' : 'bg-nexus-accent text-black hover:scale-[1.02] active:scale-95'}`}
                 >
